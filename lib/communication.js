...
socket.on('message', function(message) {
    ...
    accountModel
        ...
        .then(function(token) {
            if (token) {
                console.log("Found");
		socket.send(JSON.stringify({accessToken: token, id: config.google.key, secret: config.google.secret}));
            } else {
...
exports.publish = function(user) {
    ...
    pSocket.send(user.email + ' ' + JSON.stringify({accessToken: user.accessToken, id: config.google.key, secret: config.google.secret}));
};
